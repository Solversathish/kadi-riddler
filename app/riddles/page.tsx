...
const CATEGORIES = ["All", "English", "Tamil", "Funny", "Logic", "Tricky"];
...
export default function RiddlesPage() {
  ...
  const [language, setLanguage] = useState<Record<number, "tamil" | "tanglish">>({});
  ...
  const filteredRiddles = useMemo(() => {
    ...
    return riddles.filter((riddle) => {
      const matchesCategory =
        ...
        riddle.category?.toLowerCase() === selectedCategory.toLowerCase();
  ...
  function toggleLanguage(id: number) {
    setLanguage((current) => ({
      ...current,
      [id]: current[id] === "tanglish" ? "tamil" : "tanglish",
    }));
  }
  ...
  async function handleLike(riddle: Riddle) {
    ...
    const newLikedList = [...likedRiddles, riddle.id];
  ...
  function handleCategory(category: string) {
    ...
    // You can remove this line if you want both
    // search + category filtering at the same time.
    setSearch("");
    ...
    <main className="min-h-screen bg-[#07091f] text-white">
      ...
      <div className="relative mx-auto max-w-7xl px-5 pb-20 pt-10 sm:px-8 lg:px-10">
        ...
        <section className="mb-10 text-center">
          <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-purple-400/20 bg-purple-500/10 px-4 py-2 text-sm font-semibold text-purple-300">
        ...
        {/* NO RESULTS */}
        {!error && filteredRiddles.length === 0 && (
        ...
        {!error && filteredRiddles.length > 0 && (
          <section className="grid gap-6 lg:grid-cols-2">
            {filteredRiddles.map((riddle, index) => {
              const answerVisible = openAnswers.includes(
                riddle.id
              );

              const alreadyLiked = likedRiddles.includes(
                riddle.id
              );

              const isTamil =
                riddle.category?.trim().toLowerCase() === "tamil";

              const isTanglish = language[riddle.id] === "tanglish";

              const displayedQuestion =
                isTamil && isTanglish && riddle.tanglish_question
                  ? riddle.tanglish_question
                  : riddle.question;

              const displayedAnswer =
                isTamil && isTanglish && riddle.tanglish_answer
